...
for x in data:
    matches = re.findall(
        r'^(\w+)\swould\s(lose|gain)\s(\d+)\shappiness\sunits\sby\ssitting\snext\sto\s(\w+).$', x)
    if matches[0][0] not in guests:
        guests[matches[0][0]] = {'Me': 0}
        guests['Me'][matches[0][0]] = 0
...
def calcTotChange(order):
    ...
    for i in range(len(order)):
        if i == len(order) - 1:
            ...
            tot += guests[order[i]][order[i + 1]] + \
                guests[order[i + 1]][order[i]]
